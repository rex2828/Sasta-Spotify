...
- For improving user experience Spotify enables to add 25 users and we can add more if we request them.So what it takes, is an email of a user's spotify account and add it to there client id, so what u have to do by creating your own developer account and changing the code , normal users won't have to that much, we just need their email and they are good to go.
